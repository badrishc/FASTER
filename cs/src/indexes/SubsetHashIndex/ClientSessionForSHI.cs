...
{
    /// <summary>
    /// A SubsetHashIndex-enabled wrapper around a <see cref="ClientSession{Key, Value, Input, Output, TContext, Functions}"/>
    /// </summary>
    public sealed class ClientSessionForSHI<TKVKey, TKVValue, TInput, TOutput, TContext, TFunctions> : IDisposable
#if DEBUG
        ...
        where TFunctions : IAdvancedFunctions<TKVKey, TKVValue, TInput, TOutput, TContext>
    {
        ...
        private readonly ClientSessionSHI<FasterKVProviderData<TKVKey, TKVValue>, long> indexSession;

        internal const string NotAsyncSessionErr = ClientSession<int, int, int, int, Empty, SimpleFunctions<int, int>>.NotAsyncSessionErr;

        internal ClientSessionForSHI(FasterKV<TKVKey, TKVValue> fkv, IndexingFunctions<TKVKey, TKVValue, TInput, TOutput, TContext, TFunctions> indexingFunctions,
                                  ...
                                  SubsetHashIndex<FasterKVProviderData<TKVKey, TKVValue>, long> subsetHashIndex)
        {
            ...
            this.indexSession = subsetHashIndex.NewSession();
        }

        private void EnterIndexableOperation(IndexableOperation indexableOp)
        {
            if (this.indexingFunctions.IsSet)
                throw new InvalidOperationExceptionSHI("Cannot execute concurrent actions on a session");
        ...
        /// </summary>
        /// <example>
        /// foreach (var providerData in fkvShi.Query(sizePred, Size.Medium)) {...}
        /// </example>
        /// <typeparam name="TPKey">The type of the key value to return results for</typeparam>
        /// <param name="pred">The Predicate object</param>
        /// <param name="key">The key value to return results for</param>
        ...
        /// </summary>
        /// <example>
        /// foreach (var providerData in fkvShi.Query(sizePred, new TesTPKey[] { Size.Medium, Size.Large })) {...}
        ...
        /// </summary>
        /// <example>
        /// var providerData in fkvShi.Query(sizePred, Size.Medium, colorPred, Color.Red, (l, r) => l || r))
        /// </example>
        ...
        /// </summary>
        /// <example>
        /// foreach (var providerData in fkvShi.Query(
        ...
        /// </summary>
        /// <example>
        /// var providerData in fkvShi.Query(sizePred, Size.Medium, colorPred, Color.Red, countPred, 7, (l, m, r) => l || m || r))
        /// </example>
        /// <typeparam name="TPKey1">The type of the key value for the first <see cref="Predicate{TPKey, TRecordId}"/></typeparam>
        /// <typeparam name="TPKey2">The type of the key value for the second <see cref="Predicate{TPKey, TRecordId}"/></typeparam>
            ...
            if (this.DetachTrackers(out var trackers))
            {
                foreach (var tracker in trackers)
                {
                    Status status = tracker.UpdateOp switch
                    {
                        ...
                        _ => throw new InternalErrorExceptionSHI("Unexpected UpdateOperation"),
                    };
