...
namespace FASTER.libraries.SubsetHashIndex
{
    ...
    public class SubsetHashIndex<TProviderData, TRecordId> where TRecordId : struct, IComparable<TRecordId>
    {
        ...
        private readonly Dictionary<long, ClientSessionSHI<TProviderData, TRecordId>> indexSessions = new Dictionary<long, ClientSessionSHI<TProviderData, TRecordId>>();
        ...
        /// <summary>
        /// Create a new session for ShubsetHashIndex operations.
        /// </summary>
        public ClientSessionSHI<TProviderData, TRecordId> NewSession()
        {
            var sId = Interlocked.Increment(ref NextSessionId) - 1;
            var session = new ClientSessionSHI<TProviderData, TRecordId>(this, sId);
        ...
        internal void ReleaseSession(ClientSessionSHI<TProviderData, TRecordId> session)
        {
            lock (this.indexSessions)
                this.indexSessions.Remove(session.Id);
        }

        internal Status Upsert(ClientSessionSHI<TProviderData, TRecordId> indexSession, TProviderData data, TRecordId recordId,
        ...
        internal Status Update(ClientSessionSHI<TProviderData, TRecordId> indexSession, ChangeTracker<TProviderData, TRecordId> changeTracker)
        {
            foreach (var group in this.groups.Values)
            {
        ...
        internal ValueTask UpdateAsync(ClientSessionSHI<TProviderData, TRecordId> indexSession, ChangeTracker<TProviderData, TRecordId> changeTracker, 
        ...
        internal Status Delete(ClientSessionSHI<TProviderData, TRecordId> indexSession, ChangeTracker<TProviderData, TRecordId> changeTracker)
        {
            foreach (var group in this.groups.Values)
            {
        ...
        internal ValueTask DeleteAsync(ClientSessionSHI<TProviderData, TRecordId> indexSession, ChangeTracker<TProviderData, TRecordId> changeTracker,
                                       CancellationToken cancellationToken)
        ...
        internal bool CompletePending(ClientSessionSHI<TProviderData, TRecordId> indexSession, bool spinWait = false, bool spinWaitForCommit = false)
        ...
        internal ValueTask CompletePendingAsync(ClientSessionSHI<TProviderData, TRecordId> indexSession, bool waitForCommit = false, CancellationToken cancellationToken = default) 
        ...
        internal ValueTask ReadyToCompletePendingAsync(ClientSessionSHI<TProviderData, TRecordId> indexSession, CancellationToken cancellationToken = default) 
        ...
        internal ValueTask WaitForCommitAsync(ClientSessionSHI<TProviderData, TRecordId> indexSession, CancellationToken cancellationToken = default) 
        ...
        private void VerifyIsBlittable<TPKey>()
        {
            if (!Utility.IsBlittable<TPKey>())
                throw new ArgumentExceptionSHI("The Predicate Key type must be blittable.");
        }

        private Predicate<TPKey, TRecordId> GetImplementingPredicate<TPKey>(IPredicate iPred)
        {
            if (iPred is null)
                throw new ArgumentExceptionSHI($"The Predicate cannot be null.");
            var pred = iPred as Predicate<TPKey, TRecordId>;
            Guid id = default;
            if (pred is null || !this.predicateNames.TryGetValue(pred.Name, out id) || id != pred.Id)
                throw new ArgumentExceptionSHI($"The Predicate {pred.Name} with Id {(pred is null ? "(unavailable)" : id.ToString())} is not registered with this FasterKV.");
            return pred;
        }
        ...
        private void VerifyIsOurPredicate(params IPredicate[] preds)
        {
            foreach (var pred in preds)
            {
                ...
                if (!this.predicateNames.ContainsKey(pred.Name))
                    throw new ArgumentExceptionSHI($"The Predicate {pred.Name} is not registered with this Index.");
            }
        }
        ...
        private void VerifyIsOurPredicate<TPKey>(IEnumerable<(IPredicate, IEnumerable<TPKey>)> predsAndKeys)
        {
            if (predsAndKeys is null)
                throw new ArgumentExceptionSHI($"The Predicate enumerable cannot be null.");
        ...
        private static void VerifyRegistrationSettings<TPKey>(RegistrationSettings<TPKey> registrationSettings) where TPKey : struct
        {
            if (registrationSettings is null)
                throw new ArgumentExceptionSHI("RegistrationSettings is required");
            if (registrationSettings.LogSettings is null)
                throw new ArgumentExceptionSHI("RegistrationSettings.LogSettings is required");
            if (registrationSettings.CheckpointSettings is null)
                throw new ArgumentExceptionSHI("RegistrationSettings.CheckpointSettings is required");

            // TODOdcr: Support ReadCache and CopyReadsToTail for SubsetHashIndex
            if (registrationSettings.LogSettings.ReadCacheSettings is {} || registrationSettings.LogSettings.CopyReadsToTail)
                throw new ArgumentExceptionSHI("SubsetHashIndex does not support ReadCache or CopyReadsToTail");
        }
            ...
            where TPKey : struct
        {
            ...
            if (def is null)
                throw new ArgumentExceptionSHI("Predicate definition cannot be null");
            ...
            lock (this.predicateNames)
            {
                if (predicateNames.ContainsKey(def.Name))
                    throw new ArgumentExceptionSHI($"A Predicate named {def.Name} is already registered in another group");
            ...
            where TPKey : struct
        {
            ...
            if (defs is null || defs.Length == 0 || defs.Any(def => def is null) || defs.Length == 0)
                throw new ArgumentExceptionSHI("Predicate definitions cannot be null or empty");
            ...
            if (defs.Length > Constants.kInvalidPredicateOrdinal)
                throw new ArgumentExceptionSHI($"There can be no more than {Constants.kInvalidPredicateOrdinal} Predicates in a single Group");
            const int maxKeySize = 256;
            if (Utility.GetSize(default(KeyPointer<TPKey>)) > maxKeySize)
                throw new ArgumentExceptionSHI($"The size of the Predicate key can be no more than {maxKeySize} bytes");
                ...
                for (var ii = 0; ii < defs.Length; ++ii)
                {
                    ...
                    for (var jj = ii + 1; jj < defs.Length; ++jj)
                    {
                        if (defs[jj].Name == def.Name)
                            throw new ArgumentExceptionSHI($"The Predicate name {def.Name} cannot be specfied twice");
                    }
                }
        ...
        private IDisposable GetGroupSession<TPKey>(ClientSessionSHI<TProviderData, TRecordId> indexSession, Predicate<TPKey, TRecordId> pred)
            => indexSession.GetGroupSession(this.groups[pred.GroupId]);

        internal IEnumerable<TRecordId> Query<TPKey>(ClientSessionSHI<TProviderData, TRecordId> indexSession, IPredicate pred, TPKey key, QuerySettings querySettings)
            where TPKey : struct
        {
...
#if NETSTANDARD21
        internal async IAsyncEnumerable<TRecordId> QueryAsync<TPKey>(ClientSessionSHI<TProviderData, TRecordId> indexSession, IPredicate pred, TPKey key, QuerySettings querySettings)
            where TPKey : struct
        {
            var predImpl = this.GetImplementingPredicate<TPKey>(pred);
            querySettings ??= QuerySettings.Default;
        ...
        internal IEnumerable<TRecordId> Query<TPKey>(ClientSessionSHI<TProviderData, TRecordId> indexSession, IPredicate pred, IEnumerable<TPKey> keys, QuerySettings querySettings)
            where TPKey : struct
        {
...
#if NETSTANDARD21
        internal async IAsyncEnumerable<TRecordId> QueryAsync<TPKey>(ClientSessionSHI<TProviderData, TRecordId> indexSession, IPredicate pred, IEnumerable<TPKey> keys, QuerySettings querySettings)
            where TPKey : struct
        {
            this.VerifyIsOurPredicate(pred);
            querySettings ??= QuerySettings.Default;
        ...
        internal IEnumerable<TRecordId> Query<TPKey1, TPKey2>(ClientSessionSHI<TProviderData, TRecordId> indexSession,
...
#if NETSTANDARD21
        internal IAsyncEnumerable<TRecordId> QueryAsync<TPKey1, TPKey2>(ClientSessionSHI<TProviderData, TRecordId> indexSession, 
                     IPredicate pred1, TPKey1 key1,
                     IPredicate pred2, TPKey2 key2,
                    Func<bool, bool, bool> matchPredicate,
                    QuerySettings querySettings)
        ...
        internal IEnumerable<TRecordId> Query<TPKey1, TPKey2, TPKey3>(ClientSessionSHI<TProviderData, TRecordId> indexSession,
                     IPredicate pred1, TPKey1 key1,
                     IPredicate pred2, TPKey2 key2,
...
#if NETSTANDARD21
        internal IAsyncEnumerable<TRecordId> QueryAsync<TPKey1, TPKey2, TPKey3>(ClientSessionSHI<TProviderData, TRecordId> indexSession, 
                     IPredicate pred1, TPKey1 key1,
                     IPredicate pred2, TPKey2 key2,
                     IPredicate pred3, TPKey3 key3,
                    Func<bool, bool, bool, bool> matchPredicate,
        ...
        internal IEnumerable<TRecordId> Query<TPKey>(ClientSessionSHI<TProviderData, TRecordId> indexSession,
...
#if NETSTANDARD21
        internal IAsyncEnumerable<TRecordId> QueryAsync<TPKey>(ClientSessionSHI<TProviderData, TRecordId> indexSession, 
                    IEnumerable<(IPredicate pred, IEnumerable<TPKey> keys)> predAndKeys,
                    Func<bool[], bool> matchPredicate,
                    QuerySettings querySettings = null)
            where TPKey : struct
