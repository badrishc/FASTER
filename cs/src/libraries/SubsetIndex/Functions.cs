...
{
    internal unsafe partial class FasterKVSI<TPKey, TRecordId> : FasterKV<TPKey, TRecordId>
    {
        internal class Functions : IAdvancedFunctions<TPKey, TRecordId, Input, Output, Context>, IInputAccessor<Input>
        {
            ...
            private const string NotUsedForSubsetIndex = "SubsetIndex-implementing FasterKVs should not use this IFunctions method";

            #region Upserts
            public bool ConcurrentWriter(ref TPKey _, ref TRecordId src, ref TRecordId dst, long logicalAddress) => throw new InternalErrorExceptionSI(NotUsedForSubsetIndex);

            public void SingleWriter(ref TPKey _, ref TRecordId src, ref TRecordId dst, long logicalAddress) => throw new InternalErrorExceptionSI(NotUsedForSubsetIndex);

            public void UpsertCompletionCallback(ref TPKey _, ref TRecordId value, Context ctx) => throw new InternalErrorExceptionSI(NotUsedForSubsetIndex);
#endregion Upserts
            ...
            public bool NeedCopyUpdate(ref TPKey _, ref Input input, ref TRecordId value)
                => throw new InternalErrorExceptionSI(NotUsedForSubsetIndex);
