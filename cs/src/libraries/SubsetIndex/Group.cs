...
namespace FASTER.libraries.SubsetIndex
{
    ...
    {
        internal FasterKVSI<TPKey, TRecordId> fkv;
        ...
        #region Override equivalence testing for set membership
        ...
        #endregion Override equivalence testing for set membership
        ...
        /// <param name="id">The ordinal of this Group in the <see cref="SubsetIndex{TProviderData, TRecordId}"/>'s Group list.</param>
        public Group(RegistrationSettings<TPKey> regSettings, IPredicateDefinition<TProviderData, TPKey>[] defs, long id)
        {
            ...
            this.fkv = new FasterKVSI<TPKey, TRecordId>(
        ...
        public IDisposable NewSession()
            => this.fkv.For(new FasterKVSI<TPKey, TRecordId>.Functions(this.keyAccessor, this.fkv.RecordAccessor))
                       .NewSession<FasterKVSI<TPKey, TRecordId>.Functions>(threadAffinitized: false);

        private AdvancedClientSession<TPKey, TRecordId, FasterKVSI<TPKey, TRecordId>.Input, FasterKVSI<TPKey, TRecordId>.Output,
                              FasterKVSI<TPKey, TRecordId>.Context, FasterKVSI<TPKey, TRecordId>.Functions> ToFkvSession(IDisposable sessionObj)
            => sessionObj is AdvancedClientSession<TPKey, TRecordId, FasterKVSI<TPKey, TRecordId>.Input, FasterKVSI<TPKey, TRecordId>.Output, 
                                           FasterKVSI<TPKey, TRecordId>.Context, FasterKVSI<TPKey, TRecordId>.Functions> session
                ? session
                : throw new InvalidOperationExceptionSI("Expected a FASTER AdvancedClientSession");
        ...
        public Predicate<TPKey, TRecordId> this[string name]
            => Array.Find(this.Predicates, pred => pred.Name.Equals(name, StringComparison.CurrentCultureIgnoreCase))
                ?? throw new ArgumentExceptionSI("Predicate not found");
                                             ...
                                             ChangeTracker<TProviderData, TRecordId> changeTracker)
        {
            ...
            var input = new FasterKVSI<TPKey, TRecordId>.Input(this.Id, 0);
            var value = recordId;
            ...
            var context = new FasterKVSI<TPKey, TRecordId>.Context { Functions = session.functions };
            return phase switch
            {
                ...
                _ => throw new InternalErrorExceptionSI("Unknown Predicate execution Phase {phase}")
            };
        }
        ...
        private unsafe FasterKVSI<TPKey, TRecordId>.Input MakeQueryInput(int predOrdinal, ref TPKey key)
        {
            ...
            var input = new FasterKVSI<TPKey, TRecordId>.Input(this.Id, predOrdinal);
        ...
        private IEnumerable<TRecordId> Query(IDisposable sessionObj, FasterKVSI<TPKey, TRecordId>.Input input, QuerySettings querySettings)
        {
            ...
            try
            {
                do
                {
                    var output = new FasterKVSI<TPKey, TRecordId>.Output();
        ...
        private async IAsyncEnumerable<TRecordId> QueryAsync(IDisposable sessionObj, FasterKVSI<TPKey, TRecordId>.Input input, QuerySettings querySettings)
        {
            // TODOperf: if there are multiple Predicates within this group we can step through in parallel and return them
            // as a single merged stream; will require multiple TPKeys and their indexes in queryKeyPtr. Also consider
            // having TPKeys[] for a single Predicate walk through in parallel, so the FHT log memory access is sequential.
