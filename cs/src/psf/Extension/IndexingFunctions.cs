...
namespace FASTER.PSF
{
        ...
        where UserFunctions : IAdvancedFunctions<TKVKey, TKVValue, Input, Output, Context>
    {
        ...
        {
            // If the value has objects, then an in-place RMW to the data in that object will also affect BeforeData, so we must get the PSFs now for IPU. // TODOperf this is in session lock
            ...
            this.psfManager.SetBeforeData(this.ChangeTracker, CreateProviderData(ref key, ref value), logicalAddress, isIpu && this.recordAccessor.ValueHasObjects());
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void SetAfterData(ref TKVKey key, ref TKVValue value, long logicalAddress)
            => this.psfManager.SetAfterData(this.ChangeTracker, CreateProviderData(ref key, ref value), logicalAddress);
